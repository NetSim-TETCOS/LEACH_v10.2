...
/*	for (i=0; i<NUMBEROFCLUSTERS ; i++)
	{
		if(CHcount[i] == 4)
		{
			CH [i]= fn_NetSim_LEACH_AssignClusterHead(i,pstruDevicePower);
			prevCH[i] = CH[i];
			CHcount[i] = 0;
		}
		else
...
int fn_NetSim_LEACH_AssignClusterHead (int ClusterNumber,POWER **pstruDevicePower)
//Function to dynamically assign Cluster Heads based on the sensors' remaining energy.
{
	...
	MaximumPower = pstruDevicePower[DeviceId-1]->dRemainingPower;
	ClusterHeadID = DeviceId;
		
	for(i=0; i<SIZEOFCLUSTERS; i++)
	{
		DeviceId = ClusterElements[ClusterNumber][i];
		ClusterHeadPower = pstruDevicePower[DeviceId-1]->dRemainingPower;
